...
class Orbit:
    ...
    def get_pos_at_date(self, date: datetime):
        ...
        tolerance = pow(10, -6)
        while True:
            d_mean_anomaly = (mean_anomaly - (eccentric_anomaly - ((180 / np.pi)
        ...
        x_convert.append((np.cos(argument_of_perihelion) * np.cos(long_of_asc_node)) -
                         (np.sin(argument_of_perihelion) * np.sin(long_of_asc_node) * np.cos(inclination)))
        x_convert.append((0 - np.sin(argument_of_perihelion) * np.cos(long_of_asc_node)) -
                         (np.cos(argument_of_perihelion) * np.sin(long_of_asc_node) * np.cos(inclination)))

        y_convert.append((np.cos(argument_of_perihelion) * np.sin(long_of_asc_node)) - (
                    np.sin(argument_of_perihelion) * np.cos(long_of_asc_node) * np.cos(inclination)))
        y_convert.append((0 - np.sin(argument_of_perihelion) * np.sin(long_of_asc_node)) -
                         (np.cos(argument_of_perihelion) * np.cos(long_of_asc_node) * np.cos(inclination)))
        ...
        x_ecl = x_helio * x_convert[0] + y_helio * x_convert[1]
        y_ecl = x_helio * y_convert[0] + y_helio * y_convert[1]
        z_ecl = x_helio * z_convert[0] + y_helio * z_convert[1]

        #convert to equatorial coordinates
        obliquity = 23.43928
        x_eq = x_ecl
        y_eq = np.cos(obliquity * y_ecl) - np.sin(obliquity * z_ecl)
        z_eq = np.sin(obliquity * y_ecl) - np.cos(obliquity * z_ecl)
